...
describe('a text exit the player types', () => {
  ...
  /*
   * The signal `Remotes.relayMove` is wired to (2026-09-24, party relay):
   * once a room block confirms a move, the tracker reports the exact command
   * text, for any shape of move — filtering by shape (a direction versus a
   * text exit) is deliberately `Remotes`' job, not this one's.
   */
  describe('the confirmed-move signal', () => {
    it('reports the exact text of a confirmed text exit', () => {
      const t = play(
        [
          '[HP=62/KAI=4]:',
          'Intersection of Brass St. & River St.',
      ...
      expect(t.takeConfirmedMove()).toBe('go manhole');
    });

    it('reports nothing for a typo no room block ever answers', () => {
      const t = play(
        [
          '[HP=62/KAI=4]:',
          'Intersection of Brass St. & River St.',
          'Obvious exits: down',
          { send: 'go hatch' },
          // Reprinted, unasked — nothing was queued for it to answer.
          'Intersection of Brass St. & River St.',
          'Obvious exits: down'
        ],
        streetWorld()
      );
      expect(t.takeConfirmedMove()).toBeNull();
    });

    it('reports a cardinal direction too — filtering by shape is Remotes’ job', () => {
      // No text exit at all here, and no realm data either: `e` is in the
      // command table on its own, so the tracker does not need either to
      // confirm it — and does not filter by shape when it does.
      const t = play([
        'Guild Street',
        'Obvious exits: east',
        { send: 'e' },
      ...
      expect(t.takeConfirmedMove()).toBe('e');
    });

    it('is taken by the reading: a second read sees nothing left', () => {
      const t = play(
        [
          '[HP=62/KAI=4]:',
          'Intersection of Brass St. & River St.',
